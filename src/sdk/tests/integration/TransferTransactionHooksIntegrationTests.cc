...
protected:
  ...
  AccountId createAccountWithHook(const std::shared_ptr<PrivateKey>& key,
                                  ...
                                  int64_t hookId,
                                  const Client& client,
                                  const Hbar& initialBalance = Hbar(2LL))
  {
    EvmHook evmHook;
  ...
  FungibleHookCall createFungibleHookCall(int64_t hookId, int64_t gasLimit = 25000)
  {
    FungibleHookCall hookCall;
    hookCall.setHookType(FungibleHookType::PRE_TX_ALLOWANCE_HOOK);
    hookCall.setHookId(hookId);
    hookCall.setEvmHookCall(EvmHookCall().setGasLimit(gasLimit));
    return hookCall;
  }

  NftHookCall createNftHookCall(int64_t hookId, int64_t gasLimit = 25000)
  {
  ...
  void deleteTestAccount(const AccountId& accountId, const std::shared_ptr<PrivateKey>& key)
  {
  ...
  [[nodiscard]] inline const std::string& getRejectingHookBytecode() const { return mRejectingHookBytecode; }
...
private:
  ...
  const std::string mRejectingHookBytecode =
    "6080604052348015600e575f5ffd5b506107d18061001c5f395ff3fe608060405260043610610033575f3560e01c8063124d8b301461003757"
    "806394112e2f14610067578063bd0dd0b614610097575b5f5ffd5b610051600480360381019061004c91906106f2565b6100c7565b60405161"
    "005e9190610782565b60405180910390f35b610081600480360381019061007c91906106f2565b6100d2565b60405161008e9190610782565b"
    "60405180910390f35b6100b160048036038101906100ac91906106f2565b6100dd565b6040516100be9190610782565b60405180910390f35b"
...
TEST_F(TransferTransactionHooksIntegrationTests, ShouldExecuteHooksOnMultipleAccountsInSameTransfer)
{
  ...
  const AccountId operatorId = AccountId(2ULL);

  ContractId hookContractId = createHookContract(getTestHookBytecode(), getTestClient());
  AccountId receiverId1 =
    createAccountWithHook(receiverKey1, hookContractId, getTestHookId2(), getTestClient(), Hbar(1LL));
  AccountId receiverId2 =
    createAccountWithHook(receiverKey2, hookContractId, getTestHookId2(), getTestClient(), Hbar(1LL));

  FungibleHookCall hookCall1 = createFungibleHookCall(getTestHookId2());
  FungibleHookCall hookCall2 = createFungibleHookCall(getTestHookId2());
  ...
  // Clean up
  deleteTestAccount(receiverId1, receiverKey1);
  deleteTestAccount(receiverId2, receiverKey2);
}
...
TEST_F(TransferTransactionHooksIntegrationTests, ShouldTransferFungibleTokensWithAllowanceHook)
{
  ...
  ContractId hookContractId = createHookContract(getTestHookBytecode(), getTestClient());
  AccountId receiverId = createAccountWithHook(receiverKey, hookContractId, getTestHookId1(), getTestClient());
  ...
  FungibleHookCall hookCall = createFungibleHookCall(getTestHookId1());
  ...
  // Clean up
  deleteTestAccount(receiverId, receiverKey);
}
...
TEST_F(TransferTransactionHooksIntegrationTests, ShouldTransferNftWithSenderAndReceiverAllowanceHooks)
{
  ...
  ContractId hookContractId = createHookContract(getTestHookBytecode(), getTestClient());
  AccountId senderId = createAccountWithHook(senderKey, hookContractId, getTestHookId2(), getTestClient());
  AccountId receiverId = createAccountWithHook(receiverKey, hookContractId, getTestHookId2(), getTestClient());
  ...
  NftHookCall senderCall = createNftHookCall(getTestHookId2());
  NftHookCall receiverCall = createNftHookCall(getTestHookId2());
  ...
  // Clean up
  deleteTestAccount(senderId, senderKey);
  deleteTestAccount(receiverId, receiverKey);
}
...
TEST_F(TransferTransactionHooksIntegrationTests, ShouldNotTransferWhenHookReturnsFalse)
{
  ...
  ContractId hookContractId = createHookContract(getRejectingHookBytecode(), getTestClient());
  AccountId senderId = createAccountWithHook(senderKey, hookContractId, getTestHookId1(), getTestClient());
  AccountId receiverId = AccountCreateTransaction()
                           .setKeyWithoutAlias(receiverKey->getPublicKey())
                           .setInitialBalance(Hbar(10LL))
