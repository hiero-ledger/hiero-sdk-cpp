...
An alias is an AccountId derived from a public key. It can be used in
transactions before the account exists on-ledger.
...
No explicit CreateAccount transaction is used. Funding the alias both
creates and initializes the account, which can then be referenced by
