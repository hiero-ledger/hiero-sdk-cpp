...
int main(int argc, char** argv)
{
  ...
  std::cout << "  Request Timeout: "
            << std::chrono::duration_cast<std::chrono::seconds>(client.getRequestTimeout()).count() << " seconds"
            << std::endl;
  std::cout << "  gRPC Deadline: "
            << (client.getGrpcDeadline().has_value()
                  ...
                  : "not set (uses " +
                      std::to_string(std::chrono::duration_cast<std::chrono::seconds>(DEFAULT_GRPC_DEADLINE).count()) +
                      " default)")
            << " seconds" << std::endl;
  std::cout << std::endl;
  ...
  std::cout << "  gRPC Deadline: "
            << std::chrono::duration_cast<std::chrono::seconds>(client.getGrpcDeadline().value()).count() << " seconds"
            << std::endl;
  std::cout << std::endl;
  ...
  try
  {
    std::cout << "  Attempting to set request timeout to 3 seconds (less than gRPC deadline of 5 seconds)..."
              << std::endl;
  ...
  try
  {
    const AccountBalance balance = AccountBalanceQuery().setAccountId(operatorAccountId).execute(client);
